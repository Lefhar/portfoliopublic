...
class Sendcandidature extends Controller
{
    public function index()
    {
        ...
        if (!empty($aView['liste']['email'])) {
            ...
            if ($email->send()) {
                $db->table('demarcharge')->set('date', date('Y-m-d H:i:s'))->set('statut2', 1)->where('id', $aView['liste']['id'])->update();
            } else {
                $db->table('demarcharge')->set('date', date('Y-m-d H:i:s'))->set('statut2', 2)->where('id', $aView['liste']['id'])->update();
